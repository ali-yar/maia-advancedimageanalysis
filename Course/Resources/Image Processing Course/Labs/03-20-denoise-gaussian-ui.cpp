...
namespace eiid
{
	...
	bool bilateral_or_nlmeans = true;		// whether we are applying bilateral (true) or nonlocal means (false) denoising
...
int main() 
{
	...
	{	
		...
		eiid::bilateral_or_nlmeans = true;
		...
		if(eiid::bilateral_or_nlmeans == true)
		{
			cv::createTrackbar("filter_size", "denoising", &eiid::filter_size, 50, eiid::denoiseGaussian);
			cv::createTrackbar("sigma_color", "denoising", &eiid::sigma_color, 200, eiid::denoiseGaussian);
			cv::createTrackbar("sigma_space", "denoising", &eiid::sigma_space, 100, eiid::denoiseGaussian);
		}
		else
			cv::createTrackbar("sigma_nlmeans", "denoising", &eiid::sigma_nlmeans, 100, eiid::denoiseGaussian);
