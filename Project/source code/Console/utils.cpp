...
std::string Utils::dataset_path1 = "DRIVE/";
std::string Utils::dataset_path2 = "CHASEDB1/";
std::string Utils::dataset_path3 = "STARE/";

std::string imgFormats = ".png.tif.jpg.jpeg.ppm.bmp.PNG.TIF.JPG.JPEG.PPM.BMP";
...
std::vector<cv::Mat> Utils::getImagesInFolder(std::string folder, std::string ext, bool force_gray)
{
    ...
    {
        if (ext == "*") {
            std::string s = f.substr(f.find_last_of('.'));
            if(imgFormats.find(s) == std::string::npos)
                continue;
        } else if (f.find(ext) == std::string::npos) {
            continue;
        }
...
cv::Point2i Utils::findOpticDisk(const cv::Mat &image, int searchSize)
{
    ...
    // leave only thickest vessels
    cv::medianBlur(image,im,27);

    // limit the search to around the center
    im.rowRange(0,rows/3).setTo(0);
    im.rowRange(2*rows/3,rows-1).setTo(0);

    int max = 0;
    for (int y=0; y<im.rows; y+=5){
        for (int x=0; x<im.rows; x++){

            // discard off limit areas
            ...
            // compute total white pixels
            int sum = cv::sum(win)[0]/255;

            // decide if this is the new highest density area
            if (sum > max) {
                max = sum;
                // save the center of the area
